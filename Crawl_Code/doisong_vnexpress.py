...
class QuotesSpider(scrapy.Spider):
	...
	# ]
	start_urls=['https://vnexpress.net/doi-song-p{}'.format(i) for i in range(2,95)]
	def parse(self, response):
		link1 = response.xpath('//article[@class="item-news item-news-common"]/h2[@class="title-news"]/a/@href').getall()
...
#		pass
		...
		if content is not None:
			...
			f = open('F:/Desktop/Hoctap/DeepLearning/Doisong/'+nameFile,'wb')
			f.write(text)
			f.close()
