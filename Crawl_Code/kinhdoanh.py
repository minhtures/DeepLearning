...
class QuotesSpider(scrapy.Spider):
	...
	# ]
	start_urls=['https://vnexpress.net/kinh-doanh-p{}'.format(i) for i in range(1,175)]
	def parse(self, response):
		links= response.xpath('//article[@class="item-news item-news-common"]/h2[@class="title-news"]/a/@href').getall()
		for link in links:
#			print(link)
...
#		print(len(content1))
		...
		if content is not None:
			...
			f = open('F:/Desktop/Hoctap/DeepLearning/Kinhdoanh/'+nameFile,'wb')
			f.write(text)
			f.close()
